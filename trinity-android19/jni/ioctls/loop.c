...
static const struct ioctl loop_ioctls[] = {
  IOCTL(LOOP_SET_FD, 0),
  IOCTL(LOOP_CLR_FD, 0),
  IOCTL(LOOP_SET_STATUS, 0),
  IOCTL(LOOP_GET_STATUS, 0),
  IOCTL(LOOP_SET_STATUS64, 0),
  IOCTL(LOOP_GET_STATUS64, 0),
  IOCTL(LOOP_CHANGE_FD, 0),
  IOCTL(LOOP_SET_CAPACITY, 0),

#ifdef LOOP_CTL_ADD
  IOCTL(LOOP_CTL_ADD, 0),
#endif
#ifdef LOOP_CTL_REMOVE
  IOCTL(LOOP_CTL_REMOVE, 0),
#endif
